$(document).ready(() => {
    ...
    $("#hidePoliticsSwitch").click(() => {
        chrome.storage.sync.set({ 'fmkFilter::hidePolitics': !!$("#hidePoliticsSwitch:checked").val() });
    });
...
function initRender() {
    chrome.storage.sync.get(['fmkFilter::filterMode', 'fmkFilter::hideToday', 'fmkFilter::hidePolitics'], (result) => {
